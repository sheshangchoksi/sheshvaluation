...
def create_historical_chart_matplotlib(financials):
    """Create historical chart using matplotlib"""
    ...
    ax.plot(years, revenues, label='Revenue', color='#2E86AB', linewidth=3, marker='o')
    if ebitdas:
        ax.plot(years, ebitdas, label='EBITDA', color='#06A77D', linewidth=3, marker='s')
    
    ax.set_title('Historical Financial Performance', fontsize=14, fontweight='bold')
    ax.set_xlabel('Year', fontsize=12)
    ax.set_ylabel('₹ Lacs', fontsize=12)
...
def create_projection_chart_matplotlib(projections):
    """Create projection chart using matplotlib"""
    ...
    color1 = '#4ECDC4'
    ax1.bar(years, revenues, color=color1, alpha=0.7, label='Revenue')
    ax1.set_xlabel('Year', fontsize=12)
    ax1.set_ylabel('Revenue (₹ Lacs)', fontsize=12, color=color1)
    ax1.tick_params(axis='y', labelcolor=color1)
    
    if fcfs:
        ax2 = ax1.twinx()
        color2 = '#FF6B6B'
        ax2.plot(years, fcfs, color=color2, linewidth=3, marker='o', label='FCF')
        ax2.set_ylabel('FCF (₹ Lacs)', fontsize=12, color=color2)
        ax2.tick_params(axis='y', labelcolor=color2)
    
    ax1.set_title('5-Year Financial Projections', fontsize=14, fontweight='bold')
...
def create_wacc_chart_matplotlib(wacc_details):
    """Create WACC pie chart using matplotlib"""
    sizes = [wacc_details.get('we', 0), wacc_details.get('wd', 0)]
    ...
    ax.set_title(f"Capital Structure (WACC: {wacc_details.get('wacc', 0):.2f}%)", 
...
def generate_comprehensive_pdf(data_dict):
    ...
    if financials and 'years' in financials:
        ...
        # Chart
        hist_img = create_historical_chart_matplotlib(financials)
        if hist_img:
    ...
    if projections and 'years' in projections:
        ...
        # Chart
        proj_img = create_projection_chart_matplotlib(projections)
        if proj_img:
    ...
    if wacc_details:
        # Chart
        wacc_img = create_wacc_chart_matplotlib(wacc_details)
        if wacc_img:
